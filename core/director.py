...
{{
    ...
    def generate_storyboard_for_chapter(self, project_id: str, chapter_id: str) -> List[Shot]:
        """
        为指定章节生成分镜
        ...
        if not chapter or chapter.project_id != project_id:
            raise InvalidProjectStateError(f"Chapter {chapter_id} not found in project {project_id}")
        ...
        # 获取该章节在项目中的序号偏移
        earlier_shots_count = self.session.exec(
            select(Shot).where(
                Shot.project_id == project_id,
                Shot.chapter_id != chapter_id
            )
        ).all()
        base_order = len([s for s in earlier_shots_count if s.sequence_order is not None])
        
        shots = []
        for idx, raw_shot in enumerate(raw_shots):
            shot = Shot(
                project_id=project_id,
                chapter_id=chapter_id,  # 关联章节
        ...
        if project.status == ProjectStatus.DRAFT or project.status == ProjectStatus.ASSETS_READY:
            project.status = ProjectStatus.STORYBOARD_READY
        project.updated_at = datetime.utcnow()
        self.session.add(project)
